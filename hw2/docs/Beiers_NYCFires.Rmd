...
Lastly, I created map visualizations of the duration the fire incident and the time it took for a unit to arrive by distance to the nearest fire station. I differentiated by the incident's distance from the nearest fire station and colored each property by the duration of the incident. The maps make it clear that there are far fewer locations that are even more than a half mile away from a Fire Station as opposed to less than half a mile (and even more properties are within a quarter mile from a Fire Station!). Moreover, fire units tend to arrive in a quick manner regardless of their distance to the incident. This relieved my anxieties more than anything else, but, nothing else too revealing is gleaned from this visualization. The duration of a fire must depend on other aspects I didn't have access to in this dataset. 
...
When broken out into boroughs, it seems like even though Brooklyn and Queens have many fires, a lot of them are short lived. Staten Island seems to have had the fewest fires, though some lasted quite a while. 
```{r, message = FALSE}
library(rgdal)
...
ggmap(map_bor) + 
  ...
  scale_color_brewer(name = "Length of Fire:", 
                     ...
                     guide = "legend") +
    labs(x = '', y = '', 
       title = "NYC Fires & Duration of Incident", 
       subtitle = "By Borough: 2015") +
  theme_tufte() +
  theme(axis.text = element_blank()) +
  theme(axis.ticks = element_blank())
...
No matter the borough, most fire incidents are within a half mile of a fire station! We're all safe. 
```{r}
ggmap(map_bor) + 
    geom_polygon(aes(fill = Borough, x = long, y = lat, group = group), 
                 data = nyc_bor,
                 ...
                 size = 0.2) +
    geom_point(data = full_fire[!is.na(full_fire$distance_rd), ], 
               aes(x = lon, y = lat, color = distance_rd),
                 alpha = 0.9, 
                 size = 0.3) +
    scale_color_brewer(name = "Distance to Fire Station:", 
                     type = "seq", 
                     palette = "YlOrRd", 
                     direction = 1, 
                     guide = "legend") +
    labs(x = '', y = '', 
       title = "2015 NYC Fires & Proximity to Nearest Fire Station", 
       subtitle = "By Borough: 2015") +
  theme_tufte() +
  theme(axis.text = element_blank()) +
  theme(axis.ticks = element_blank())
