...
##### Summary of how many winter games each country medaled in, and how many medals of each type the country won. 
```{r}
...
```
![image](../figs/olympmeds.png)

##### Total Medal Visualizations 
To visualize the total amount of medals won by the top 10 countries over time, I produced a bar chart with the same gold, silver and bronze coloring to make the stacks immediately obvious to the reader. 
```{r}
# Total medals
top10 %>% 
...
The below viz shows the countries lined up by population size and compares how many medals they've won over time. Countries with the fewest medals are "shamed" by being highlighted in yellow. 
```{r}
# Pop Viz 
by_pop_gdp %>% 
...
The following viz points out China, specifically, as a country with both a lot of people and a lot of money, but we can see from the size if it's bubble that China is not a very big medal producer. They could be doing better!
```{r}
by_pop_gdp %>% 
...
I then split the athletes by From the plots, we can see that women dominate in Cross Country Skiing while men dominate in the Biathlon. 
```{r, warning = False, message = False}
## Multiplot !!  
 top_10_F %>% 
  ggplot(aes(reorder(x = Athlete, totalmedals), y = totalmedals)) +
...
### Interactivity
#### Plot 1: Medals per Population
For my first interactive plot, I chose to add more information to my bubble chart. Before, the scale for total medals won was unclear. The below plot allows the user to hover over each bubble and get a clearer picture of how many overall medals the country won. 
```{r}
library(plotly)
