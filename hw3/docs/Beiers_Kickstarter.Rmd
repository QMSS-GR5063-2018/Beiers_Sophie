...
blurb_words$syllable <- syllable_sum(blurb_words$word)
blurb_words$syllable[is.na(blurb_words$syllable)] <- 0
...
blurb_words <- merge(blurb_words, projects, by = "id")

# calculating flesch reading ease scores:
blurb_words$fk_score <- (206.835 - (1.015*(blurb_words$total_words/blurb_words$sentences)) - (84.6*(blurb_words$total_syllables/blurb_words$total_words)))

# summarizing by ids
na.omit(fk <- blurb_words %>% 
  group_by(id) %>% 
  select(id, fk_score) %>% 
  unique())
...
hchart(fk, "scatter", hcaes(fk_score, backers_count, size = backers_count, color = color)) %>% 
  ...
  hc_xAxis(
    title = list(text = "Flesch-Kinkaid Score (easy to difficult)", gridLineWidth = 0.5)) %>% 
  hc_add_theme(hc_theme_tufte())
```
